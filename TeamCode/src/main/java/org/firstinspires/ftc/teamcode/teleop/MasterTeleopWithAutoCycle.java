...
@Disabled
@TeleOp(name = "Master Tele-op Auto Cycle", group = "Tele-op")
public class MasterTeleopWithAutoCycle extends OpMode {
    ...
    void autoUpdate()
    {
        ...
        if(gamepad1.start && !prevStart && !auto.isPathing() && outtake.getState().equals(OuttakeConstants.GRAB_SPECIMEN_READY)) {
            auto.startAuto();
        } else if (Math.abs(gamepad1.left_stick_x) + Math.abs(gamepad1.left_stick_y) > 0.05 && auto.isPathing()) {
            auto.stopAuto();
        }

        auto.increment(gamepad1);
